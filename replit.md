...
- **Activity Badge Instant Reset**: Admin activity badges reset immediately on login by using `previous_login_at` timestamp rotation (done atomically in auth-service.ts loginUser function) and cache-busted API calls that depend on `user?.id` for login transition detection
