...
- **Cart Session Isolation System**: Implemented secure cart ownership model with `carts` database table (owner_type: 'guest'|'user', owner_id). Cart DAL with ownership validation, API endpoints with server-side guards, guest-to-user cart merge on login/registration, cart clearing on logout. Prevents cross-user data leakage.
- **Cart Merge on Auth**: Both `loginViaAPI` and `registerViaAPI` call `/api/cart/merge` after successful auth, then `syncWithServer()` to load merged cart. Cart store syncs on mount via `isSynced` flag. Rollback on server rejection for all cart operations.
